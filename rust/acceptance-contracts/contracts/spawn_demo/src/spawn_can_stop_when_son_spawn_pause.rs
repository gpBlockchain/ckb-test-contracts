...
use ckb_std::syscalls::{current_cycles, debug, process_id};
...
pub fn program_entry() -> i8 {
    ...
    if process_id() == 1 {
        ...
        let mut spgs = syscalls::SpawnArgs {
            ...
            inherited_fds: std_fds.as_ptr(),
        };
        print_current_cycle();
        syscalls::spawn(0, Source::CellDep, 0, 0, &mut spgs).unwrap();
    ...
    match syscalls::wait(spawn_result1) {
        Ok(ok) => {
            debug(format!("wait:{:?} succ ok:{:?}", spawn_result1, ok));
        }
        Err(err) => {
            debug(format!("failed:{:?}", err));
        }
    }
