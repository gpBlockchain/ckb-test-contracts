...
///  argc  != argv.len()
///
///     case1 :  argv.len() = 1, argc == 26214
///     result :
///         exit_code = 0
///         spawn return = 0
///
///     case2 : argv.len() = 1, argc.len() == 99999
///     result :
///         [contract debug] panicked at 'memory allocation of 262144 bytes failed', library/alloc/src/alloc.rs:415:13
///         exit_code = -1
///         spawn return = 0
///
///
pub fn program_entry() -> i8 {
    ...
    // case1 :  argv.len() = 1, argc == 26214
    let result = spawn_func(0, Source::CellDep, 0, 26214, cstrs.as_slice(), &spawn_args);
    debug!("exit:{:?}",exit_code);
    assert_eq!(exit_code, 0);
    assert_eq!(result, 0);
    // case2 :  argv.len() = 1, argc == 99999
    let result = spawn_func(0, Source::CellDep, 0, 99999, cstrs.as_slice(), &spawn_args);
    debug!("exit:{:?}",exit_code);
